...
/// TARGET_DOC_SUBFOLDER is the path fragment for the subfolder
/// in the Target Dir containing documentation generated by rustdoc.
const TARGET_DOC_SUBFOLDER: &'static str = "doc";

/// Fetch the default location for documentation. This is
/// CARGO_TARGET_DIR + /doc/
fn doc_location() -> PathBuf {
    // TARGET_DIR_ENV is the name of the enviroment variable
    // containing Cargo's target directory (where it
    // outputs build artifacts).
    let out_dir = std::option_env!("OUT_DIR").unwrap_or("./target/");
    Path::new(out_dir).join(TARGET_DOC_SUBFOLDER)
}
